...
import org.seekloud.theia.protocol.ptcl.CommonInfo.{ClientType, RoomInfo, UserDes}
import org.seekloud.theia.webClient.common.{Page, Routes}
import org.seekloud.theia.webClient.util.{Globals, Http, JsFunc, ScalaWebSocket}
...
class LivePage(userId:Long, roomId:Long) extends Page{
  ...
  private val anchorInfo = {
    <div class="showInfo">
      <img src={
           if(isTemUser())
             if(dom.window.sessionStorage.getItem("coverImgUrl") != null)
               dom.window.sessionStorage.getItem("coverImgUrl")
             else dom.window.localStorage.getItem("coverImgUrl")
           else
             if(dom.window.localStorage.getItem("coverImgUrl") != null)
                dom.window.localStorage.getItem("coverImgUrl")
             else
                dom.window.sessionStorage.getItem("coverImgUrl")} class="showInfo-coverImg"></img>
      <div style="margin-left:20px;height:80px;margin-top:10px">
        <div style="color:#2f3e4e;font-size:20px">{
          dom.window.sessionStorage.getItem("roomName")}</div>
    ...
    Http.postJsonAndParse[SearchRoomRsp](Routes.UserRoutes.searchRoom,data).map{
      case Right(rsp) =>
        if(rsp.errCode==0 && rsp.roomInfo.isDefined){
          ...
          val url = dom.window.location.protocol + "//" + dom.window.location.hostname + ":30389"+ rsp.roomInfo.get.mpd.getOrElse("")
          dashurl = url
...
//          }
  ...
  def connectWebsocket(): Unit = {
    val (userId, token, roomId) =
      if(isTemUser()) {
        (dom.window.sessionStorage.getItem("userId").toLong,
          dom.window.sessionStorage.getItem("token").toString,
          dom.window.sessionStorage.getItem("roomId").toLong)
      }
      else {
    ...
    val wsurl = rmWebScocketUri(userId, token, roomId)
            ...
            if(i=="dash"){
              ...
              <div>
                ...
                <div class="loading ccl-panel">
                  <img src="/theia/roomManager/static/img/loading2.png"  class="load2" id="load2"></img>
                </div>
                <div class="ccl-panel">
                  <div class="abp">
