...
import org.seekloud.theia.webClient.common.PageSwitcher
import mhtml._
...
import org.seekloud.theia.protocol.ptcl.client2Manager.http.AdminProtocol._
import org.seekloud.theia.protocol.ptcl._
import org.seekloud.theia.protocol.ptcl.rtmp2Manager.RtmpProtocol.{GetTokenReq, GetTokenRsp}
...
object MainPage extends PageSwitcher {
  ...
  private val exitButton: Elem =
    <div class="header-exit" onclick={()=> dom.window.location.hash = "#/Home"}>
      <img src="/theia/roomManager/static/img/logo.png" title="主页"></img>
      <div>主页</div>
    </div>
...
//    Var(adminShow)
    ...
    currentHashVar.map { current =>
      ...
      current match{
        ...
        case "Live" :: roomId :: Nil =>
          ...
          val prePage = new LivePage(if(isTemUser()) dom.window.sessionStorage.getItem("userId").toLong else dom.window.localStorage.getItem("userId").toLong,roomId.toLong)
  ...
  def clearRoomInfo() = {
    dom.window.sessionStorage.removeItem("roomName")
    if(dom.window.sessionStorage.getItem("roomId") != null){
      dom.window.sessionStorage.removeItem("roomId")
      dom.window.sessionStorage.removeItem("coverImgUrl")
      dom.window.sessionStorage.removeItem("headImgUrl")
    }
  ...
  def clearRecordInfo()={
    dom.window.sessionStorage.removeItem("recordName")
    dom.window.sessionStorage.removeItem("recordCoverImg")
    dom.window.sessionStorage.removeItem("recordStartTime")
  }
  def downLoad(e: dom.Event)={
    dom.window.open(s"/theia/roomManager/file/download/$fileName")
    e.stopPropagation()
  }
  //main--------------------------------------------
  def show(): Cancelable = {
    ...
    val page =
      <div>
        ...
        <div class="header-backgroudImg">
          <img src="/theia/roomManager/static/img/header_1.jpeg" style="width:100px;height:100px"></img>
          <img src="/theia/roomManager/static/img/header_2.jpeg" style="width:100px;height:100px;margin-top:50px"></img>
          <img src="/theia/roomManager/static/img/header_3.jpeg" style="width:140px;height:140px;margin-top:-20px"></img>
          <img src="/theia/roomManager/static/img/header_5.jpeg" style="width:100px;height:100px;margin-top:40px"></img>
        </div>
        {currentPage}
      </div>
  ...
  def register(e: Event, popId: String): Unit = {
    ...
    if (!emial.trim.equals("") && !account.trim.equals("") && !password.trim.equals("") && !password2.trim.equals("")) {
      if (password.equals(password2)) {
        PopWindow.registerButton := <img src="/theia/roomManager/static/img/loading.gif"></img>
        val redirectUrl = s"http://${dom.document.location.host}/theia/webClient"
      ...
      case Right(rsp) =>
        if(rsp.errCode == 0){
          rsp.userInfoOpt.foreach{ userNewInfo =>
            dom.window.sessionStorage.setItem("userName", userNewInfo.userName)
            dom.window.sessionStorage.setItem("userHeaderImgUrl", userNewInfo.headImgUrl)
            dom.window.sessionStorage.setItem("userId", userNewInfo.userId.toString)
            dom.window.sessionStorage.setItem("token", userNewInfo.token.toString)
...
//            }
  ...
  def emailLogin(e: Event, popId: String): Unit = {
    PopWindow.emailLoginButton := <img src="/theia/roomManager/static/img/loading.gif"></img>
  ...
  def refresh() = {
    ...
    if(dom.window.location.hash.contains("Live")){
//      val condition =
//          (dom.window.sessionStorage.getItem("userName") == null && dom.window.localStorage.getItem("userName") == null) && (dom.window.sessionStorage.getItem("roomId") != null || dom.window.localStorage.getItem("roomId") != null)
      if(dom.window.localStorage.getItem("userName") == null){

        val roomId = dom.window.localStorage.getItem("roomId")
        if(roomId != null) dom.window.sessionStorage.setItem("roomId", roomId)

        temUserLogin(dom.window.sessionStorage.getItem("roomId").toLong)
      }
      else{
        dom.window.location.reload()
      }
    ...
    if(dom.window.location.hash.contains("Record")){
      if(dom.window.sessionStorage.getItem("userName") == null && dom.window.localStorage.getItem("userName") == null){
        temUserLogin(-1l)
      }
      else{
        openOrClose()
//        dom.window.location.reload()
  ...
  def goLive = {
    dom.window.location.hash = s"#/Live/${dom.window.sessionStorage.getItem("roomId")}"
  }
  ...
  def connectRTMP(): Unit = {
    ...
    val data = GetTokenReq(userId).asJson.noSpaces
